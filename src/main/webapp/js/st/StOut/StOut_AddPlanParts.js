...
function   initFrame()
{
    ...
    if(receptionId!=null&&receptionId!=''){ 
        $('#stout_addplanparts_receptionId').attr("value",receptionId);
        query();
        $('#hiddenRecptionId').val(receptionId); 
        $('#so_planParts').datagrid('options').url='StOutAction!loadFrtReceptParts.action?receptionId='+receptionId;
   	    $('#so_planParts').datagrid('reload');
        //var data=$('#so_planParts').datagrid('getData');
		    }
		}
...
$('#so_frtReceptionTable').datagrid({
     ...
     pageSize : 5,
		...
		}, {title : '维修班组',field : 'pregrpName',width : 60
       ...
       onDblClickRow:function(rowIndex, rowData){
 	     ...
 	     $('#so_planParts').datagrid("options").url='StOutAction!loadFrtReceptParts.action?receptionId='+rowData.receptionId;
   	     $('#so_planParts').datagrid("reload");
   	     var data=$('#so_planParts').datagrid('getRows');
   	     dbClickState='dbClick';
 	   },
 	   onLoadSuccess:function(rowIndex, rowData){
		  var receptionId=$('#receptionId').val();
	      if(trim(receptionId)!=null&&trim(receptionId)!=''){ 
	    	$('#so_frtReceptionTable').datagrid('selectRow',0);
	      }
   	   }
    });
 
	//计划用料信息
	$('#so_planParts').datagrid({
		url : '',
		fit:false,
		height:200,
		width:760,
		...
		rownumbers : true,
		 columns:[[{field:'ck',checkbox:true
			...
			}, {title : '收费性质编号',field : 'itemChargeId',hidden:true
...
function query(){
	 $.ajax({   
			...
			data : $('#stout_selectplanpartsform').serialize(),
			dataType : "json",
			success : function callback(r) {
    	    	$('#so_frtReceptionTable').datagrid('loadData',r);
            }
     });
}
...
function _clear(){
	...
	$('#so_planParts').datagrid('loadData', {total:0,rows:[]});
}