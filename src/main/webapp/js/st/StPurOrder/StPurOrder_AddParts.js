  ...
  $selectedParts.datagrid('loadData',detailData);
   ...
   function copyPartsDateAndBindEvent(id, rowIndex, rowData)
   {
		...
		ed[0].target.bind('blur', function() {
			...
			var noTaxPrice = ed[1].target.val();
			if(noTaxPrice==null||noTaxPrice==''||parseFloat(noTaxPrice)<=0||isNaN(noTaxPrice)){
				alertMsg('采购金额不能为空,非法字符或者小于等于零！', 'warning');
				ed[1].target.numberbox('setValue', '1.00');
				noTaxPrice=1.00;
		    }
			var noTaxAmount = accMul(parseFloat(num), parseFloat(noTaxPrice));//自定义计算总额方法
			ed[2].target.numberbox('setValue', noTaxAmount);//赋给指定列
            var taxPrice=accMul(parseFloat(taxRate),parseFloat(noTaxPrice));
            ed[3].target.numberbox('setValue', taxPrice);//赋给指定列
            var taxAmount=accMul(parseFloat(num),parseFloat(taxPrice));
		...
		ed[1].target.bind('blur', function() {
			...
			ed[2].target.numberbox('setValue', noTaxAmount);
	        var taxPrice=accMul(parseFloat(taxRate),parseFloat(noTaxPrice));
	        ed[3].target.numberbox('setValue', taxPrice);//赋给指定列
            var taxAmount=accMul(parseFloat(num),parseFloat(taxPrice));
            ed[4].target.numberbox('setValue', taxAmount);//赋给指定列
