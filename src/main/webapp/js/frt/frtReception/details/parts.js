$(function (){
	if(staticResvId!=""){
		$('#frtReceptionPartsDatagrid').datagrid({
			url : 'frtReceptionAction!findPartsByResvId.action?resvId=' + staticResvId
		});
	}else{
		if(receptionId){
			$('#frtReceptionPartsDatagrid').datagrid({
				url : 'frtReceptionAction!findPartsByRcptId.action?receptionId=' +receptionId
			});
		}else{
			$('#frtReceptionPartsDatagrid').datagrid({
				url : 'frtReceptionAction!findPartsByRcptId.action?receptionId=-1'
			});
		}
	}
	//前台接车单->计划材料
