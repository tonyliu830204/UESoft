$(function (){
	...
	companyInfomationSetDatagrid.datagrid({
		url : 'basCompanyInformationSetAction!findCompanyAll.action',
		singleSelect : true,
		fit : true,
		pagination : true,
		fitColumns : true,
...
var _clear = function (){
    ...
    $('#companyInfomationSetQueryForm').form('clear');
	companyInfomationSetDatagrid.datagrid('load', serializeObject($('#companyInfomationSetQueryForm')));
}
//导出
function _except(){
	showEditDialog("companyInfomationSetDatagrid",null,"companyInfomationSetDatagrid_center","开始导出","导出配置",0,_callbackExcept);
}
function _callbackExcept(parentId,fieldNames){
	exportEsuyUIExcelFile(parentId,fieldNames,"公司信息设定"+getSystemTime());
}