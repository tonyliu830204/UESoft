...
var loadComboTree = function(){
	$('#instoreTree').treegrid({
		url : 'retreatAction!getQueryBack.action',
		rownumbers:true,
		pagination : true,
		pageSize : 10,
		...
		},
		{
			...
			//动态设置展开查询的url
			var url = 'retreatAction!getBack.action?carModelId='+rowData.carModelId+'&'+$('#carModelQueryForm').serialize();
			$('#instoreTree').treegrid("options").url = url;
			return true;
	    },onLoadSuccess:function(row, data){
	    	initTreeGridPager('instoreTree', 'retreatAction!getQueryBack.action', true);
	    } 
	});
}
//清空
function clearaa(){
    ...
    reLoadTreeGrid('instoreTree', 'carModelQueryForm', 'retreatAction!getQueryBack.action', true, true);
...
function queryHouse (){
	...
	reLoadTreeGrid('instoreTree', 'carModelQueryForm', 'retreatAction!getQueryBack.action', false, true);
	addInitDate($('#reDateStart'),$('#repyDateEnd'));
}
...
exportEsuyUIExcelFile(parentId,fieldNames,"退厂单查询"+getSystemTime());
}
