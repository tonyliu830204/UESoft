...
<html>
  <head>
    ...
    <script type="text/javascript">
    	var staticFlag=true;
		var stfid = "<%=((BasUsers)session.getAttribute(Contstants.CUSTOMER)).getBasStuff().getStfId() %>";
		var tc = "<%=Contstants.SELL_BILLSDEPLOY.BACKTOCAR %>";
    	//退款记录
		$(function(){
			//初试时间
			...
			$('#datagrid_refundment_manage_id').datagrid({
				url : encodeURI('refundmentAction!getRefundmentInfo.action?editTag=false&prefix='+tc+''),
				pagination : true,
				fit : true,
				rownumbers : true,
				singleSelect : true,
				...
				onClickRow:function(rowIndex, rowData){
					...
					$('#datagrid_refundment_manage_id_detail_id').datagrid('options').url='refundmentAction!getRefundmentInfo.action?editTag=true&backCar_Document='+rowData.backCar_Document+'&prefix='+tc+'';
			...
			$('#datagrid_refundment_manage_id_detail_id').datagrid({
				...
				onLoadSuccess:function(){
					$('#datagrid_refundment_manage_id_detail_id').datagrid('options').url='refundmentAction!getRefundmentInfo.action?editTag=true&prefix='+tc+'';
				}
			});
		});
    	//
			...
			if(data){
				...
				$.ajax({
					...
					success : function(r){
							...
							$('#invoiceNum').val('');
							$('#backMoneyPerson').combobox('setValue',stfid);
		...
		function doRefundment(){
			if($('#form_dilog_refundment_manage_id').form('validate')){
				...
				$.ajax({
					type : 'POST',
					url : 'refundmentAction!saveRefundmentAmount.action?prefix='+tc+'',
						...
						$.ajax({
						   ...
						   success: function(r){
							 if(r.success){
								...
								'refundmentAction!getRefundmentInfo.action?editTag=true&backCar_Document='+datas.backCar_Document+'&prefix='+tc+'';
								$('#datagrid_refundment_manage_id_detail_id').datagrid('reload');
								$('#datagrid_refundment_manage_id').datagrid('reload');
							 }else{
								 alertMsg(r);
							...
							$.ajax({
							   ...
							   success: function(r){
								 if(r.success){
									 $('#datagrid_refundment_manage_id').datagrid('reload');
									 $('#datagrid_refundment_manage_id_detail_id').datagrid('unselectAll');
									$('#datagrid_refundment_manage_id_detail_id').datagrid('options').url='refundmentAction!getRefundmentInfo.action?editTag=true&backCar_Document='+data.backCar_Document+'&prefix='+tc+'';
									$('#datagrid_refundment_manage_id_detail_id').datagrid('reload');
								 }else{
									 alertMsg(r);
								 }
	...
	var updateRefundment=function(){
		if($('#form_dilog_refundment_manage_id').form('validate')){
			...
			$.ajax({
				type : 'POST',
				url : 'refundmentAction!updateRefundmentAmount.action?prefix='+tc+'',
				data : '&'+$('#form_dilog_refundment_manage_id').serialize(),
			    dataType : 'json',
				success : function(r){
					if(r.success){
