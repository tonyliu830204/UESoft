...
$(function() {
	//初始化时间
	 addInitDate($('#tracing_Date'),$('#tracing_Date2'));
$('#account').datagrid({
...
function queryReserve() {
	...
	$('#account').datagrid('load',serializeObjectByflag($('#queryForm'),false));
	//初始化时间
	 addInitDate($('#tracing_Date'),$('#tracing_Date2'));
...
function clearSearchCondition(){
	...
	$('#account').datagrid(
			{
				url : 'possibleCustomFollowAction!getTzCustom.action?flag=false'
			})
	/*$('#account').datagrid('load', serializeObjectByflag($('#queryForm'),false));	*/	
...
function queryToday(){
	...
	$.ajax( { 
		type : 'POST',
		url : 'possibleCustomFollowAction!getTzCustom.action',
		data: "times=1&flag=true",
		dataType : 'json',
		success : function(r) {		
			if (r) {
				$('#account').datagrid('loadData',r);
/*			$('#account')
					.datagrid({
								url : 'possibleCustomFollowAction!getTzCustom.action?times=1&flag=true',
								pagination : true,
								});*/
