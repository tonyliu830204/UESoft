  ...
  <body class="easyui-layout" >
   ...
   //判断页面初始化加载完成    执行
			...
			$('#distributorId').datagrid({
					...
					columns : [ [ {
			      	 ...
			      	 onDblClickRow:function(rowIndex, rowData){     
						$('#dName2').val(rowData.distributorName);
						$('#did2').val(rowData.distributorId);
	...
	var queryCarModel = function (){
		...
		$('#distributorId').datagrid('load', serializeObject($('#aae')) );
}
function clearSearchff() {
	 	$('#aae').find('input').val('');
		$('#aae').find('select').val('');
		$('#distributorId').datagrid('unselectAll');
		$('#distributorId').datagrid('load', serializeObject($('#aae')));
		...
		<div id="cc" class="easyui-layout" fit="true" border="false">
			<div data-options="region:'north',title:'查询条件'" style="overflow: hidden; padding: 1px; background: #eee; height: 53px;" border="false">
				<form id="aae" name="aae" method="post" fit="true">
					名称:<input type="text" id="dNamew" name="distributorName" style="background-color: #c0d8d8;width: 200px" onkeyup="queryCarModel()" />
					&nbsp;&nbsp;<a id='_clear' href="javascript:void(0);" class="easyui-linkbutton"
										iconCls="icon-cancel" plain="true"  onclick="clearSearchff();">清空</a>
