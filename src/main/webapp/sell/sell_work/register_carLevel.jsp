...
<html>
  <head>
	...
	$(function (){
		...
		remoteSort : false,
		singleSelect : true,
		frozenColumns : [[{field : 'leva_name',title : '级别',width : 100,sortable : true, align : 'center'}]],
		...
		function doFindbyCondition(datagrid,formid){
			...
			//同时访问 后台getName方法筛选出需要的列
				$.ajax({
				type : 'POST', 
				url : 'backRegisterAction!doRegisterCarLevel.action?flag=true',
				data : formvalue,
				success : function(rest){
					if(rest && rest!=null){
						...
						if(t){
							$.ajax({
							type : 'POST', 
							url : 'backRegisterAction!getRegisterCarLevel.action',
							data : formvalue,
							dataType : 'json',
							success : function(resault){
								var object=JSON.parse(resault);
								$('#datagrid_register_carLevel_id').datagrid('loadData',{rows:[]});			
								$('#datagrid_register_carLevel_id').datagrid('loadData',object);			
							}
						});
					}
					}
		...
		//导出
		function doExport(){
		var data =  $('#datagrid_register_carLevel_id').datagrid('getData'); 
		 if(data.rows.length==0){
			 alertMsg('数据列表为空，不能导出！', 'warning');
			 return ;
		 }
		 	showEditDialog('datagrid_register_carLevel_id',null,'datagrid_register_user_carmodel_div',"开始导出","导出配置",0,_callbackExcept);
			
		}
		function _callbackExcept(parentId,fieldNames){
		
			exportEsuyUIExcelFile(parentId,fieldNames,"客流车级别分析"+getSystemTime());
		}
		//打印	
		function doPoint(){
		var data =  $('#datagrid_register_carLevel_id').datagrid('getData'); 
		 if(data.rows.length==0){
			...
			showEditDialog('datagrid_register_carLevel_id',null,'datagrid_register_user_carmodel_div',"开始打印","打印配置",2,_print);
		}
		function _print(parentId,fieldNames){
  		...
  		<div class="easyui-layout" style="width:1000px;height:800px;" border="false" fit="true">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-print" plain="true" id='_print' onclick="doPoint();" >打印</a>
		...
		<div region="center" style="background:#eee;"  border="false">
			...
			<div data-options="region:'north',collapsible : false" style="overflow: hidden;padding:3px; background:#eee; height:90px;" border="false">
				...
				<div id="datagrid_register_user_carmodel_div" region="center" border="false" >
