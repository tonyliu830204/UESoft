...
<html>
  <head>
	...
	$(function (){
		...
		remoteSort : false,
		singleSelect : true,
		frozenColumns : [[{field : 'messagechannel',title : '渠道',width : 100,sortable : true, align : 'center'}]],
		onLoadSuccess : function(data){
		if(data.total == '0'){
			var body = $(this).data().datagrid.dc.body2;
			body.find('table tbody').append('<tr><td width="'+body.width()+'" style="height: 25px; text-align: center;" colspan="6">请选择来店登记日期查询客流车来源情况！</td></tr>');
					}
				}
		...
		function doFindbyCondition(datagrid,formid){
			...
			//同时访问 后台getName方法筛选出需要的列
				$.ajax({
				type : 'POST', 
				url : 'backRegisterAction!doRegisterCustomSource.action?flag=true',
				data : formvalue,
				success : function(rest){
					if(rest && rest!=null){
						...
						if(t){
							$.ajax({
							type : 'POST', 
							url : 'backRegisterAction!getRegisterCustomSource.action',
							data : formvalue,
							dataType : 'json',
		...
		function _callbackExcept(parentId,fieldNames){
			exportEsuyUIExcelFile(parentId,fieldNames,"客流车来源分析"+getSystemTime());
		}
		//打印
		...
		function _print(parentId,fieldNames){
  		...
  		<div class="easyui-layout" style="width:1000px;height:800px;" border="false" fit="true">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-print" plain="true" id='_print' onclick="dopoint('datagrid_register_carLevel_id','datagrid_register_user_carmodel_div');" >打印</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-export" plain="true" id='_export' onclick="doexcept('datagrid_register_carLevel_id','datagrid_register_user_carmodel_div');" >导出</a>
			</div>
