...
<html>
  <head>
    <title>接待员车品牌分析</title>
    <script type="text/javascript">
	$(function (){
		//初试时间
		addInitDate($('#register_Date'),$('#register_Date2'));
		//初始化
		$('#datagrid_register_carLevel_id').datagrid({
		url : '',
		type : 'POST',
		newrap : false,
		fitColumns : true,
		...
		remoteSort : false,
		singleSelect : true,
		frozenColumns : [[{field : 'stfname',title : '业务员',width : 100,sortable : true, align : 'center'}]],
		onLoadSuccess : function(data){
		if(data.total == '0'){
			var body = $(this).data().datagrid.dc.body2;
			body.find('table tbody').append('<tr><td width="'+body.width()+'" style="height: 25px; text-align: center;" colspan="6">请选择来店登记日期查询客流车级别情况！</td></tr>');
					}
				}
		...
		for ( var i = 0; i < array.length; i++) {
			...
			str2 = str2 + ",{title : '"+str+"',field : 'brand"+str+"',width : 100,sortable : true, align : 'center'} ";
			}
		...
		var laststr = str1 + newstr2 + str3;
			 ...
			 options.columns = eval(laststr);
				 $('#datagrid_register_carLevel_id').datagrid(options);
			return true;	 
		}
		//条件查询
		function doFindbyCondition(datagrid,formid){
			...
			//同时访问 后台getName方法筛选出需要的列
				$.ajax({
				type : 'POST', 
				url : 'backRegisterAction!doRegisterCarBrand.action?flag=true',
				data : formvalue,
				success : function(rest){
					if(rest && rest!=null){
						...
						if(t){
							$.ajax({
							type : 'POST', 
							url : 'backRegisterAction!getRegisterCarBrand.action',
							data : formvalue,
							dataType : 'json',
							success : function(resault){
								var obj = JSON.parse(resault);
								$('#datagrid_register_carLevel_id').datagrid('loadData',{rows:[]});
								$('#datagrid_register_carLevel_id').datagrid('loadData',obj);			
							}
						});
					}
					}
		...
		var doFormClear = function(){
			$('#form_register_carLevel_id').form('clear');
			doFindbyCondition($('#datagrid_register_carLevel_id'),$('#form_register_carLevel_id'));
		}
		//导出
		...
		function _callbackExcept(parentId,fieldNames){
			exportEsuyUIExcelFile(parentId,fieldNames,"接待员车品牌分析"+getSystemTime());
		}
		//打印
		...
		function _print(parentId,fieldNames){
  		...
  		<div class="easyui-layout" style="width:1000px;height:800px;" border="false" fit="true">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-search" plain="true" id='_clear' onclick="doFindbyCondition($('#datagrid_register_carLevel_id'),$('#form_register_carLevel_id'));">查询</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" id='_clear' onclick="doFormClear();">清空</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-print" plain="true" id='_print' onclick="dopoint('datagrid_register_carLevel_id','datagrid_register_carLevel_div');" >打印</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-export" plain="true" id='_export' onclick="doexcept('datagrid_register_carLevel_id','datagrid_register_carLevel_div');" >导出</a>
			</div>
		...
		<div region="center" style="background:#eee;"  border="false">
			...
			<div data-options="region:'north',collapsible : false" style="overflow: hidden;padding:3px; background:#eee; height:90px;" border="false">
				<form id="form_register_carLevel_id">
				<fieldset>
					<legend>查询条件</legend>
				...
				<div id="datagrid_register_carLevel_div" region="center" border="false" >
					<table id="datagrid_register_carLevel_id"></table>
				</div>
			</div>
		</div>
	</div>	
