...
<html>
  <head>
	...
	$(function (){
		...
		//初始化
		$('#datagrid_register_worker_id').datagrid({
		url : '',
		type : 'POST',
		newrap : false,
		fitColumns : true,
		...
		var str2 = "";
		var resaults = resault.substring(1,resault.indexOf("]"));
		if(resaults!=null && resaults!="" ){
		str2 = "[[ {title : '登记日期', field : 'registerdate',width : 100,sortable : true, align : 'center'} ";
		var array = resault.substring(1,resault.indexOf("]")).split(",");
		for ( var i = 0; i < array.length; i++) {
		var str = array[i].substring(1,array[i].length-1).split("#");
			str2 += ",{title : '"+str[1]+"',field : 'stfname"+str[0]+"',width : 100,sortable : true, align : 'center'} ";
		}
			 str2 +=  ",{title : '合计',field : 'sums',width : 100,sortable : true, align : 'center'} ]]";
			 options = {};
			 options.url = '';
			 options.columns = eval(str2);
			 $('#datagrid_register_worker_id').datagrid(options);
				return true;
			}else{
			 options = {};
			 options.url = '';
			 options.columns = eval(str2);
			 $('#datagrid_register_worker_id').datagrid(options);
			 $('#datagrid_register_worker_id').datagrid('loadData',{rows:[]});			
				return false;
			}	 
		}
		//条件查询
		function doFindbyCondition(datagrid,formid){
			...
			//同时访问 后台getName方法筛选出需要的列
				$.ajax({
				type : 'POST', 
				url : 'backRegisterAction!doRegisterWorker.action?flag=true',
				data : formvalue,
				success : function(rest){
						...
						if(t){
							$.ajax({
							type : 'POST', 
							url : 'backRegisterAction!getRegisterWorker.action',
							data : formvalue,
							dataType : 'json',
							success : function(resault){
								var obj = JSON.parse(resault);
								$('#datagrid_register_worker_id').datagrid('loadData',{rows:[]});			
								$('#datagrid_register_worker_id').datagrid('loadData',obj);			
							}
						});
					}
				}
		...
		var doFormClear = function(){
			$('#form_register_worker_id').form('clear');
			doFindbyCondition($('#datagrid_register_worker_id'),$('#form_register_worker_id'));
		}
	...
	function _print(parentId,fieldNames){
		printEsuyUIPreview(parentId,fieldNames);
  		...
  		<div class="easyui-layout" style="width:1000px;height:800px;" border="false" fit="true">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-search" plain="true" id='_clear' onclick="doFindbyCondition($('#datagrid_register_worker_id'),$('#form_register_worker_id'))">查询</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" id='_clear' onclick="doFormClear();">清空</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-print" plain="true" id='_print' onclick="dopoint('datagrid_register_worker_id','datagrid_register_worker_div');" >打印</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-export" plain="true" id='_export' onclick="doexcept('datagrid_register_worker_id','datagrid_register_worker_div');" >导出</a>
			</div>
		...
		<div region="center" style="background:#eee;"  border="false">
			...
			<div data-options="region:'north',collapsible : false" style="overflow: hidden;padding:3px; background:#eee; height:90px;" border="false">
				<form id="form_register_worker_id">
				<fieldset>
					<legend>查询条件</legend>
				...
				<div id="datagrid_register_worker_div" region="center" border="false" >
					<table id="datagrid_register_worker_id"></table>
				</div>
			</div>
		</div>
	</div>	
