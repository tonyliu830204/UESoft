...
<html>
  <head>
	...
	$(function (){
		...
		//初始化
		$('#datagrid_register_user_carmodel_id').datagrid({
		url : '',
		type : 'POST',
		newrap : false,
		fitColumns : true,
		...
		var laststr = str1 + newstr2 + str3;
			 ...
			 options.columns = eval(laststr);
				 $('#datagrid_register_user_carmodel_id').datagrid(options);
			return true;	 
		}
		//条件查询
		function doFindbyCondition(datagrid,formid){
			...
			//同时访问 后台getName方法筛选出需要的列
				$.ajax({
				type : 'POST', 
				url : 'backRegisterAction!doRegisterUserCarmodel.action?flag=true',
				data : formvalue,
				success : function(rest){
					if(rest && rest!=null){
						...
						if(t){
							$.ajax({
							type : 'POST', 
							url : 'backRegisterAction!getRegisterUserCarmodel.action',
							data : formvalue,
							dataType : 'json',
							success : function(resault){
								var obj = JSON.parse(resault);
								$('#datagrid_register_user_carmodel_id').datagrid('loadData',{rows:[]});			
								$('#datagrid_register_user_carmodel_id').datagrid('loadData',obj);			
							}
						});
					}
					}
		...
		var doFormClear = function(){
			$('#form_register_user_carmodel_id').form('clear');
			doFindbyCondition($('#datagrid_register_user_carmodel_id'),$('#form_register_user_carmodel_id'));
		}
	...
	function _callbackExcept(parentId,fieldNames){
		exportEsuyUIExcelFile(parentId,fieldNames,"业务员车型分析"+getSystemTime());
	}
	//打印
	...
	function _print(parentId,fieldNames){
		printEsuyUIPreview(parentId,fieldNames);
  		...
  		<div class="easyui-layout" style="width:1000px;height:800px;" border="false" fit="true">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-search" plain="true" id='_clear' onclick="doFindbyCondition($('#datagrid_register_user_carmodel_id'),$('#form_register_user_carmodel_id'))">查询</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" id='_clear' onclick="doFormClear();">清空</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-print" plain="true" id='_print' onclick="dopoint('datagrid_register_user_carmodel_id','datagrid_register_user_carmodel_div');" >打印</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-export" plain="true" id='_export' onclick="doexcept('datagrid_register_user_carmodel_id','datagrid_register_user_carmodel_div');" >导出</a>
			</div>
		...
		<div region="center" style="background:#eee;"  border="false">
			...
			<div data-options="region:'north',collapsible : false" style="overflow: hidden;padding:3px; background:#eee; height:90px;" border="false">
				<form id="form_register_user_carmodel_id">
				<fieldset>
					<legend>查询条件</legend>
				...
				<div id="datagrid_register_user_carmodel_div" region="center" border="false" >
					<table id="datagrid_register_user_carmodel_id"></table>
				</div>
			</div>
		</div>
	</div>	
